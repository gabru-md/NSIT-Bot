...
class NSITBot(object):
    ...
    def generateResponse(self,botPredict):
        ...
        if prediction in self.hello_bot:
            ...
            res = self.getStreamBasedResponse(message_tolower,prediction)
            self.response = str(res[1]) + " : " + str(res[0])
    ...
    def getBest_QueryID(self,message_topredict):
        ...
        """
        queryDataFrame = pd.read_csv('status.csv')
    ...
    def getBest_Response(self,queryID,queryAuthor):
        ...
        commentDataFrame = pd.read_csv('comments.csv')
...
#print bot.getStreamBasedResponse(str(raw_input("Question / Stream Based: ")),"stream_based")
